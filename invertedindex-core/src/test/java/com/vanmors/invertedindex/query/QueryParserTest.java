...
class QueryParserTest {
    ...
    @Test
    void parseBinaryNot() {
        Query q = QueryParser.parse("cat NOT dog");
        assertThat(q).isInstanceOf(AndQuery.class);
        AndQuery and = (AndQuery) q;
        assertThat(and.children()).hasSize(2);
        assertThat(and.children().get(0)).isInstanceOf(TermQuery.class);
        assertThat(((TermQuery) and.children().get(0)).term()).isEqualTo("cat");
        assertThat(and.children().get(1)).isInstanceOf(NotQuery.class);
        assertThat(((TermQuery) ((NotQuery) and.children().get(1)).child()).term()).isEqualTo("dog");
    }

    @Test
    void binaryNotEquivalentToAndNot() {
        Query a = QueryParser.parse("blue AND mountain NOT (blue NEAR/1 mountain)");
        Query b = QueryParser.parse("blue AND mountain AND NOT (blue NEAR/1 mountain)");
        assertThat(a).usingRecursiveComparison().isEqualTo(b);
    }

    @Test
    void parseMixedAndNotOperators() {
        Query q = QueryParser.parse("a AND b NOT c AND d");
        assertThat(q).isInstanceOf(AndQuery.class);
        AndQuery and = (AndQuery) q;
        assertThat(and.children()).hasSize(4);
        assertThat(and.children().get(0)).isInstanceOf(TermQuery.class);
        assertThat(and.children().get(1)).isInstanceOf(TermQuery.class);
        assertThat(and.children().get(2)).isInstanceOf(NotQuery.class);
        assertThat(and.children().get(3)).isInstanceOf(TermQuery.class);
    }
