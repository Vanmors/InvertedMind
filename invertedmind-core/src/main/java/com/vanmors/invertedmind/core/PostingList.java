...
import com.vanmors.invertedmind.codec.DeltaTransform;
...
public final class PostingList {
    ...
    public static PostingList build(java.util.List<Posting> postings, PForDeltaCodec codec, int skipInterval) {
        ...
        for (int i = 0; i < skipCount; i++) {
            int chunkEnd = (i + 1) * skip;
            int chunkLen = chunkEnd - chunkStart;
            com.vanmors.invertedmind.util.VarIntUtil.writeVInts(freqBuf, freqs, chunkStart, chunkLen);
            chunkStart = chunkEnd;
            skipFreqOffs[i] = freqBuf.position();
        }
        if (chunkStart < df) {
            com.vanmors.invertedmind.util.VarIntUtil.writeVInts(freqBuf, freqs, chunkStart, df - chunkStart);
        }
        freqBuf.flip();
        ...
        for (int i = 0; i < df; i++) {
            ...
            com.vanmors.invertedmind.codec.DeltaTransform.encode(pos, 0, pos.length);
            com.vanmors.invertedmind.util.VarIntUtil.writeVInt(posBuf, pos.length);
            for (int v : pos) {
                com.vanmors.invertedmind.util.VarIntUtil.writeVInt(posBuf, v);
            }
        }
        ...
        for (int s = 0; s < skipCount; s++) {
            if (skipPosOffs[s] == 0 && skipCumCounts[s] > 0) {
                ...
                for (int d = 0; d < targetDoc; d++) {
                    int cnt = com.vanmors.invertedmind.util.VarIntUtil.readVInt(scan);
                    for (int j = 0; j < cnt; j++) {
                        com.vanmors.invertedmind.util.VarIntUtil.readVInt(scan);
                    }
                }
